...
import { ArrowLeft, ArrowRight, Mail, RefreshCw, Sparkles } from "lucide-react";
...
function safeCallbackURL(value: string | null) {
  try {
    ...
    return url.origin === new URL(dashboardHome).origin ? url.toString() : dashboardHome;
  } catch {
...
export default function VerifyEmailPage() {
  ...
  const verifyEmail = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      setErrorMessage(error.message ?? "That verification code is invalid or expired.");
      setLoading(null);
      return;
    }
  ...
  const resendCode = async () => {
    ...
    if (error) setErrorMessage(error.message ?? "Unable to send another code.");
    else setSent(true);
    setLoading(null);
  };
      ...
      <section className="relative z-10 mx-auto grid min-h-[calc(100vh-76px)] max-w-[1160px] items-center gap-16 px-5 pb-16 pt-6 sm:px-8 lg:grid-cols-[1fr_460px] lg:pb-24">
        ...
        <div className="auth-card reveal w-full rounded-[24px] border border-white/[0.075] bg-[#10130f]/90 p-5 shadow-[0_35px_100px_rgba(0,0,0,0.4)] backdrop-blur-xl sm:p-8">
          <div className="mb-7">
            ...
            <p className="mt-2 text-[13px] leading-6 text-white/40">We sent a 6-digit code to <span className="font-medium text-white/70">{email || "your email"}</span>.</p>
          </div>
          ...
          <form onSubmit={verifyEmail} className="space-y-4">
            <label className="block"><span className="mb-2 block text-[12px] font-medium text-white/65">Verification code</span><input required value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" placeholder="000000" className="auth-input text-center font-mono tracking-[0.45em]" /></label>
            {errorMessage && <p role="alert" className="text-xs text-red-300">{errorMessage}</p>}
            <button type="submit" disabled={!email || code.length !== 6 || loading !== null} className="auth-submit">{loading === "verify" ? <RefreshCw size={15} className="animate-spin" /> : <>Verify and continue<ArrowRight size={15} /></>}</button>
          </form>
          <button type="button" disabled={!email || loading !== null} onClick={resendCode} className="mt-3 h-11 w-full rounded-xl border border-white/[0.075] bg-white/[0.025] text-xs font-medium text-white/55 transition hover:bg-white/[0.05] hover:text-white">{loading === "resend" ? "Sending code..." : sent ? "Code sent" : "Resend verification code"}</button>
          <p className="mt-5 text-center text-[10px] leading-5 text-white/30">Codes expire after 10 minutes.</p>
