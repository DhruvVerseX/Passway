...
export function ControlPlaneShell({
  ...
  return (
    <div className="min-h-screen bg-[#0b0d0b] text-white selection:bg-[#b9f55d]/25">
      ...
      />
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-[224px] flex-col border-r border-white/[0.07] bg-[#0d0f0c] transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex h-[62px] items-center gap-3 border-b border-white/[0.065] px-5">
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setSidebarOpen(false)}
                  className={`flex h-9 items-center gap-2.5 rounded-lg px-2.5 text-xs font-medium transition ${isActive ? "bg-white/[0.065] text-white" : "text-white/42 hover:bg-white/[0.035] hover:text-white/75"}`}
                >
                  <Icon
                    size={15}
                    strokeWidth={isActive ? 2.2 : 1.8}
                    className={isActive ? "text-[#b9f55d]" : ""}
                  />
                  {item.label}
                  {item.count && (
        ...
        <div className="flex items-center gap-2 border-t border-white/[0.065] p-3 text-white/35">
          <LifeBuoy size={14} />
          <a
      ...
      <div className="lg:pl-[224px]">
        <header className="sticky top-0 z-20 flex h-[62px] items-center border-b border-white/[0.065] bg-[#0b0d0b]/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          <button
          ...
          <div className="flex min-w-0 items-center gap-2 text-xs">
            ...
            <span className="truncate font-medium text-white/75">{title}</span>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <button className="hidden h-9 items-center gap-2 rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 text-xs text-white/40 transition hover:bg-white/[0.04] hover:text-white/70 md:flex">
              <Search size={14} />
              <span>Search</span>
              <span className="ml-3 rounded border border-white/[0.08] bg-white/[0.035] px-1.5 py-0.5 font-mono text-[9px] text-white/25">
                ⌘ K
              </span>
            </button>
            <button
        ...
        <main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-5 lg:px-6 lg:py-7">
          {children}
        </main>
      </div>
    </div>
