...
export function ControlPlaneShell({ children, active = "Overview", title = "Passway Cloud", showCreate = true }: ControlPlaneShellProps) {
  ...
  return <div className="min-h-screen bg-[#0b0d0b] text-white selection:bg-[#b9f55d]/25">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[224px] flex-col border-r border-white/[0.07] bg-[#0d0f0c] transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex h-[62px] items-center gap-3 border-b border-white/[0.065] px-5"><Link href="/dashboard" onClick={() => setSidebarOpen(false)}><img src="/assets/logo/passway-logo-dark.svg" alt="Passway" className="h-8 w-auto" /></Link><span className="rounded border border-white/[0.08] bg-white/[0.035] px-1.5 py-0.5 text-[9px] font-semibold tracking-wider text-white/35">BETA</span><button className="ml-auto text-white/35 transition hover:text-white lg:hidden" onClick={() => setSidebarOpen(false)} aria-label="Close navigation"><X size={18} /></button></div>
    ...
    <div className="lg:pl-[224px]"><header className="sticky top-0 z-20 flex h-[62px] items-center border-b border-white/[0.065] bg-[#0b0d0b]/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8"><button onClick={() => setSidebarOpen(true)} className="mr-3 grid h-9 w-9 place-items-center rounded-lg border border-white/[0.08] text-white/55 lg:hidden" aria-label="Open navigation"><Menu size={17} /></button><div className="flex min-w-0 items-center gap-2 text-xs"><span className="hidden text-white/30 sm:inline">Passway Cloud</span><span className="hidden text-white/20 sm:inline">/</span><span className="truncate font-medium text-white/75">{title}</span></div><div className="ml-auto flex items-center gap-2"><button className="hidden h-9 items-center gap-2 rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 text-xs text-white/40 transition hover:bg-white/[0.04] hover:text-white/70 md:flex"><Search size={14} /><span>Search</span><span className="ml-3 rounded border border-white/[0.08] bg-white/[0.035] px-1.5 py-0.5 font-mono text-[9px] text-white/25">⌘ K</span></button><button className="grid h-9 w-9 place-items-center rounded-lg border border-white/[0.08] text-white/40 transition hover:bg-white/[0.04] hover:text-white/70" aria-label="Help"><CircleHelp size={16} /></button>{showCreate && <><Link href="/dashboard/environments" className="hidden h-9 items-center gap-2 rounded-lg border border-white/[0.09] px-3 text-xs font-medium text-white/60 transition hover:bg-white/[0.04] hover:text-white sm:inline-flex"><Box size={14} /> New environment</Link><Link href="/dashboard/environments" className="inline-flex h-9 items-center gap-2 rounded-lg bg-[#b9f55d] px-3.5 text-xs font-semibold text-[#10130d] transition hover:bg-[#c8ff72] focus:outline-none focus:ring-4 focus:ring-[#b9f55d]/20"><Plus size={14} strokeWidth={2.5} /><span className="hidden sm:inline">Create Environment</span><span className="sm:hidden">Create</span></Link></>}</div></header><main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-5 lg:px-6 lg:py-7">{children}</main></div>
  </div>;
}
