...
import { Bell, Blocks, ChevronsUpDown, CircleUserRound, FolderKanban, LayoutDashboard, LifeBuoy, Settings } from "lucide-react";
...
export function AppShell({children}:Readonly<{children:React.ReactNode}>) {
  return <div className="min-h-screen bg-[#05070A] lg:grid lg:grid-cols-[240px_1fr]">
    ...
    <div className="min-w-0"><header className="flex h-16 items-center justify-between border-b border-[#1D2633] px-5 lg:px-8"><div className="flex items-center gap-2 font-semibold lg:hidden"><img src="/assets/logo/passway-logo-dark.svg" alt="Passway" className="h-8 w-auto" /></div><span className="hidden text-sm text-[#8B949E] lg:block">Workspace / <span className="text-[#E6EDF3]">Passway</span></span><div className="flex items-center gap-2"><button className="grid size-9 place-items-center rounded-lg text-[#8B949E] hover:bg-white/5" aria-label="Notifications"><Bell size={18}/></button><button className="grid size-9 place-items-center rounded-lg text-[#8B949E]" aria-label="Account menu"><CircleUserRound size={20}/></button></div></header><main className="p-5 lg:p-8">{children}</main></div>
  </div>;
}
