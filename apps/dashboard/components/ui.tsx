...
export function Card({
  ...
  return (
    <div
      className={`rounded-xl border border-[#1D2633] bg-[#0B0F14] ${className}`}
    >
      {children}
    </div>
  );
...
export function Button({
  ...
  const styles = {
    primary: "bg-[#3B82F6] text-white hover:bg-blue-500",
    secondary:
      "border border-[#1D2633] bg-[#0B0F14] text-[#E6EDF3] hover:bg-white/5",
...
export function Badge({
  ...
  children: ReactNode;
  tone?: "neutral" | "success" | "danger" | "warning" | "blue";
}) {
  const styles = {
    neutral: "border-[#1D2633] text-[#8B949E]",
...
export function EmptyState({
  ...
  return (
    <div className="grid min-h-64 place-items-center p-8 text-center">
      <div>
        <span className="mx-auto grid size-11 place-items-center rounded-xl border border-[#1D2633] bg-[#05070A] text-[#8B949E]">
          {icon}
        </span>
