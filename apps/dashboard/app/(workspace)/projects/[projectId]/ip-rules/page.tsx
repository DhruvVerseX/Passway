...
const rules = [
  ["Office network", "103.21.58.0/24"],
  ["Production VPC", "10.0.0.0/16"],
  ["Deploy runner", "198.51.100.17/32"],
];
    ...
    <ProjectPageShell projectId={projectId}>
      <div className="mb-5 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#8B949E]">
            Only approved networks can request secrets.
          </p>
        </div>
        <Button>
          <Plus size={16} />
      ...
      <Card>
        <div className="divide-y divide-[#1D2633]">
          {rules.map(([name, cidr]) => (
            <div key={cidr} className="flex items-center gap-4 p-5">
              ...
              <span className="min-w-0 flex-1">
                ...
                <code className="mono text-xs text-[#8B949E]">{cidr}</code>
              </span>
